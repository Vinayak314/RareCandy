...
def distribute_shares(bank_attributes, stock_prices):
    ...
    Small banks have concentrated portfolios (one stock dominates).
    Large banks have more diversified portfolios across stocks.
    ...
    for bank_name, attrs in bank_attributes.items():
        ...
        # Small banks (size_percentile ~ 0): alpha ~ 0.1 -> very concentrated (one stock dominates)
        # Large banks (size_percentile ~ 1): alpha ~ 2.0 -> more diversified
        ...
        if size_percentile < 0.3:  # Small banks
            ...
            primary_stock_idx = random.randint(0, num_stocks - 1)
            alphas = [base_alpha * 0.3 for _ in range(num_stocks)]  # Low base for all
            alphas[primary_stock_idx] = base_alpha * 5.0  # Primary stock gets 5x weight
        elif size_percentile < 0.6:  # Medium banks
            # Moderately concentrated: 1-2 stocks get more weight
            primary_stock_idx = random.randint(0, num_stocks - 1)
            alphas = [base_alpha for _ in range(num_stocks)]
            alphas[primary_stock_idx] = base_alpha * 2.0  # Primary gets 2x
        else:  # Large banks
