...
import networkx as nx # Added import for StrategicAnalysis
import copy # Added import for StrategicAnalysis
...
CONFIG = {
    ...
    'CRASH_SEVERITY': 1.5,  # Reduced from 3.5
    'SHOCK_RANGE': (0.01, 0.10), # Reduced from (0.05, 0.25)
...
class CCPAgent:
    ...
    def __init__(self, state_size: int = 8, num_banks: int = 50): # Modified state_size from 10 to 8
        self.state_size = state_size
...
class CCPGame:
    ...
    def __init__(self, banks_file: str, stocks_file: str, matrix_file: str):
        ...
        # Get the actual state size from network simulator for bank agents
        # Assuming there's at least one bank to get features from
        sample_bank_features = self.network.get_bank_features(self.network.bank_list[0])
        bank_state_size = len(sample_bank_features)

        # Initialize bank agents (MODEL 1)
        self.bank_agents = {
            bank: BankAgent(bank, state_size=bank_state_size) for bank in self.network.bank_list
        }

        # Initialize CCP agent (MODEL 1)
        # CCP state_size is bank_state_size (5) + 3 additional features (trade_size, predicted_risk, direction)
        ccp_state_size = bank_state_size + 3
        self.ccp_agent = CCPAgent(num_banks=len(self.network.bank_list), state_size=ccp_state_size)
    ...
    def run_episode(self, T: int = None, train: bool = True) -> Dict:
        ...
        all_ccp_features_this_timestep = [] # New list to store all 8-feature vectors for CCP
        ...
        for t in range(T):
            ...
            for bank in self.network.bank_list:
                ...
                bank_features = self.network.get_bank_features(bank) # This is 5 features
                ...
                # Construct the 8-feature input for CCP's model for this specific trade
                ccp_input_features_for_this_trade = np.concatenate([
                    bank_features, # 5 features
                    [bank_action.trade_size, predicted_risk,
                     1.0 if bank_action.direction == 'BUY' else 0.0] # 3 features
                ])
                all_ccp_features_this_timestep.append(ccp_input_features_for_this_trade) # Store it
            ...
            if train:
                # Train CCP
                # Ensure ccp_state always has consistent dimensions for remembering (8 features)
                if all_ccp_features_this_timestep:
                    # Average the 8-feature input vectors for all trades in this timestep
                    ccp_state_for_remember = np.mean(all_ccp_features_this_timestep, axis=0)
                else:
                    # If no trades, use a zero vector of the correct size (8 features)
                    ccp_state_for_remember = np.zeros(self.ccp_agent.state_size)

                # action_idx for CCP is simplified, as it's a global reward for the entire timestep
                self.ccp_agent.remember(ccp_state_for_remember, 0, ccp_reward, ccp_state_for_remember, False)
...
class StrategicAnalysis:
    """
    Advanced analytics to quantify systemic risk, compare market structures,
    and stress-test incentives.
    """

    def __init__(self, game_instance):
        self.game = game_instance
        self.matrix_df = pd.read_csv(r'C:\Users\vinayak\Desktop\RareCandy\ML\dataset\us_banks_interbank_matrix.csv', index_col=0)

    def analyze_fragility(self):
        """
        Step 1: Identify Fragile Structures & SIFIs (Systemically Important Financial Institutions).
        Uses PageRank to find 'Super-Spreaders' of contagion.
        """
        print("\n" + "="*60)
        print("🔍 STEP 1: NETWORK FRAGILITY ANALYSIS")
        print("="*60)

        # Build directed graph from interbank matrix
        G = nx.DiGraph()
        for lender in self.matrix_df.index:
            for borrower in self.matrix_df.columns:
                exposure = self.matrix_df.loc[lender, borrower]
                if exposure > 0:
                    # Contagion flows from Borrower (Failure) -> Lender (Loss)
                    G.add_edge(borrower, lender, weight=exposure)

        # 1. PageRank (DebtRank Proxy)
        pagerank = nx.pagerank(G, weight='weight')

        # 2. Total Interbank Borrowing (Direct Impact)
        total_borrowing = self.matrix_df.sum(axis=0) # Sum of columns
        ...
        for bank in G.nodes():
            results.append({
                'Bank': bank,
                'Systemic_Risk_Score': pagerank.get(bank, 0),
                'Direct_Contagion_Potential': total_borrowing.get(bank, 0)
            })

        df_risk = pd.DataFrame(results).sort_values('Systemic_Risk_Score', ascending=False)

        print("\n🏆 TOP 5 SYSTEMIC RISK SOURCES (Super-Spreaders):")
        print(df_risk[['Bank', 'Systemic_Risk_Score', 'Direct_Contagion_Potential']].head(5).to_string(index=False))

        return df_risk

    def compare_market_structures(self, episodes=50):
        """
        Step 2: The Counterfactual (CCP vs. Bilateral).
        Compares systemic loss with and without the CCP mechanism.
        """
        print("\n" + "="*60)
        print("⚖️ STEP 2: CCP vs. BILATERAL MARKET COMPARISON")
        print("="*60)

        # Scenario A: CCP Active (Standard Model)
        print(f"Running {episodes} episodes WITH CCP mechanism...")
        ccp_losses = []
        for _ in range(episodes):
            res = self.game.run_episode(train=False) # Use trained agents
            ccp_losses.append(res['systemic_loss'])

        # Scenario B: Bilateral (No Margins, Raw Exposure)
        print(f"Running {episodes} episodes WITHOUT CCP (Bilateral)...")
        bilateral_losses = []

        # Save original margin policy
        original_policy = copy.deepcopy(self.game.ccp_agent.model)

        # Monkey-patch CCP to request 0 margin (simulating bilateral credit extension)
        # In a real model, we'd also remove the Default Fund protection logic
        # For this simulation, we assume 0 margin = 100% loss propagation
        class DummyCCPAgent:
            def get_action(self, *args, **kwargs):
                return CCPAction(margin_requirement=0.0, decision=TradeDecision.APPROVED)

        real_ccp = self.game.ccp_agent
        self.game.ccp_agent = DummyCCPAgent() # Swap in dummy

        for _ in range(episodes):
            res = self.game.run_episode(train=False)
            bilateral_losses.append(res['systemic_loss'])

        # Restore real CCP
        self.game.ccp_agent = real_ccp

        # Results
        avg_ccp = np.mean(ccp_losses)
        avg_bilateral = np.mean(bilateral_losses)
        reduction = ((avg_bilateral - avg_ccp) / avg_bilateral) * 100 if avg_bilateral > 0 else 0

        print("\n📊 RESULTS:")
        print(f"  Avg Systemic Loss (WITH CCP):    ${avg_ccp:.2f}B")
        print(f"  Avg Systemic Loss (BILATERAL):   ${avg_bilateral:.2f}B")
        print(f"  🛡️ RESILIENCE VALUE ADDED:       {reduction:.1f}% Reduction in Losses")

        return {'ccp': ccp_losses, 'bilateral': bilateral_losses}

    def stress_test_incentives(self):
        """
        Step 3: Incentive Alignment Analysis.
        What happens if banks become greedy (High Volume Preference, Low Risk Aversion)?
        """
        print("\n" + "="*60)
        print("🧠 STEP 3: INCENTIVE STRESS-TEST (GREED vs. FEAR)")
        print("="*60)

        scenarios = [
            {'name': 'Balanced (Baseline)', 'lambda': 0.3, 'gamma': 0.01},
            {'name': 'Greedy (High Risk Appetite)', 'lambda': 0.01, 'gamma': 0.5},
            {'name': 'Fearful (Risk Averse)', 'lambda': 1.0, 'gamma': 0.0}
        ]

        results = []

        # Store original config
        orig_lambda = CONFIG['LAMBDA_RISK']
        orig_gamma = CONFIG['GAMMA_VOLUME']

        for scen in scenarios:
            print(f"Testing Scenario: {scen['name']}...")

            # Apply config
            CONFIG['LAMBDA_RISK'] = scen['lambda']
            CONFIG['GAMMA_VOLUME'] = scen['gamma']
            ...
            for _ in range(20):
                res = self.game.run_episode(train=False)
                losses.append(res['systemic_loss'])
                failures.append(res['failed_banks'])
        ...
        # Restore config
        CONFIG['LAMBDA_RISK'] = orig_lambda
        CONFIG['GAMMA_VOLUME'] = orig_gamma

        df_res = pd.DataFrame(results)
        print("\n📈 INCENTIVE IMPACT REPORT:")
        print(df_res.to_string(index=False))
# ============================================================================
# MAIN EXECUTION
# ============================================================================
# ============================================================================
# MAIN EXECUTION (UPDATED FOR TRAINING)
# ============================================================================
if __name__ == '__main__':
    print("=" * 70)
    print("CCP GAME-THEORETIC RISK SIMULATION - TRAINING MODE")
    print("=" * 70)

    # 1. Initialize game
    # Ensure these CSV files are in the same folder as your script!
    game = CCPGame(
        banks_file=r'C:\Users\vinayak\Desktop\RareCandy\ML\dataset\phase1_engineered_data.csv',
        stocks_file=r'C:\Users\vinayak\Desktop\RareCandy\ML\dataset\stocks_data_long.csv',
        matrix_file=r'C:\Users\vinayak\Desktop\RareCandy\ML\dataset\us_banks_interbank_matrix.csv'
    )

    # 2. FORCE TRAINING (Ignore existing files for now)
    # We increase episodes to 1000 to allow the RL agents to converge
    print("\n🚀 Starting Fresh Training Session...")
    game.train(num_episodes=1000)

    # 3. RUN ANALYSIS (The "Business Impact" Layer)
    # This uses the class we defined in the previous step
    # (Make sure you include the StrategicAnalysis class in your file!)
    if 'StrategicAnalysis' in globals():
        analyzer = StrategicAnalysis(game)

        # Step A: Network Fragility
        analyzer.analyze_fragility()

        # Step B: CCP vs Bilateral
        analyzer.compare_market_structures(episodes=50)

        # Step C: Incentive Stress Test
        analyzer.stress_test_incentives()
    else:
        print("\n⚠️ StrategicAnalysis class not found. Paste the analysis code to see business insights.")

    # 4. Save the finalized models
    game.save_models()